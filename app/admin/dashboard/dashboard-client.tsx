...
// Separate component for handling guest resume upload
function GuestResumeHandler({
  onUploadStart,
  onUploadComplete,
}: {
  onUploadStart: () => void;
  ...
  useEffect(() => {
    ...
    if (searchParams.get("uploadResume") === "true") {
  ...
  }, [searchParams, router, onUploadStart, onUploadComplete, hasChecked]);
...
export function DashboardClient({ session }: { session: Session }) {
  ...
  const handleGuestUploadStart = () => {
    setIsUploadingGuest(true);
  };

  const handleGuestUploadComplete = () => {
    setShowWelcome(true);
    setActiveTab("versions");
    setRefreshKey((prev) => prev + 1);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 dark:from-gray-900 dark:to-gray-800">
      {/* Handle guest resume upload with Suspense to prevent flash */}
      <Suspense fallback={null}>
        <GuestResumeHandler
          onUploadStart={handleGuestUploadStart}
          onUploadComplete={handleGuestUploadComplete}
        />
      </Suspense>
